...
template<typename L>
concept BackendConcept = requires(L l, std::size_t w, std::size_t h, const ege::FrameBuffer<1024>& frame, std::vector<ege::Event>& events, int sample_rate, uint32_t sound_id, float frequency, float duration, ege::Event &out) {
    ...
    { l.trigger_sound(sound_id, frequency, duration) } -> std::same_as<void>;

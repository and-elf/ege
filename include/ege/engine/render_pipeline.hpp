...
public:
    ...
    [[nodiscard]] CmdBuf& begin_frame() noexcept {
        uint32_t idx;
        assert(free_idx_q_.pop(idx));
    ...
    void submit_frame() noexcept {
        assert(current_write_idx_ == UINT32_MAX || current_write_idx_ < BufferCount);
        assert(used_idx_q_.push(current_write_idx_));
